...
def build_metadata(version: str, platform: str, arch: str, archive_name: str, size_compressed: int, size_installed: int) -> dict[str, object]:
    ...
    return {
        ...
        "url": f"{HOMEPAGE}/releases/download/{release_tag}/{archive_name}",
        "homepage": HOMEPAGE,
        "sourceUrl": HOMEPAGE,
        "buildDate": build_date,
        "tags": ["cli", "build", "orchestrator", "beez"],
...
def import_runtime_libs_from_binary_package(binary_path: Path, payload_bin_dir: Path, platform: str) -> None:
    import sys

    ci_dir = Path(__file__).resolve().parent
    if str(ci_dir) not in sys.path:
        sys.path.insert(0, str(ci_dir))
    from package_binary import collect_runtime_libs, copy_runtime_files

    copy_runtime_files(binary_path, payload_bin_dir, platform)
    collect_runtime_libs(binary_path, payload_bin_dir, platform)


def main() -> int:
    parser = argparse.ArgumentParser(description="Package Beez as ReqPack .rqp archive")
    ...
    archive_name = f"{PACKAGE_NAME}-{version}-{args.platform}-{args.arch}.rqp"
    archive_path = output_dir / archive_name

    with tempfile.TemporaryDirectory(prefix=f"beez-rqp-{args.platform}-{args.arch}-", dir=output_dir) as temp_dir:
