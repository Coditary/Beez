...
- **Plugin task imports** — import reusable tasks from plugins via `task("coditary/demo:format")` or `task("alias", { plugin = "coditary/demo", task = "format" })`. Plugins declare tasks in a `tasks { ... }` block (like `workflows`).
 `app`, `debug`, `coverage`, `sanitize`, `tsan`, `fuzz`, `test`, `lint`, `format`, `analyze`, `security`, `audit`, `docs`, `code`, `repo`. Plugin-specific scopes (`code`, `supply`, `smoke`, `cppcheck_analyze`, …) removed. `fuzz:seed-verify` is phase `verify` scope `fuzz`; SBOM/supply-chain steps use scope `audit`; publish maps to scope `app`.
