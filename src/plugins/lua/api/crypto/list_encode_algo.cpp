...
void bindListEncodeAlgo(sol::table& cryptoTable, const std::shared_ptr<sol::state>& luaState)
{
    cryptoTable["list_encode_algo"] = [luaState]() -> sol::table
    {
        return crypto_detail::algorithmsToTable(luaState,
                                                crypto_detail::supportedEncodingAlgorithms());
    };
}
