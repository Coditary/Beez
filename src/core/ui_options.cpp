...
UiSettings resolveUiSettings(const UiSettingsOverlay& overlay)
{
    ...
    settings.truecolor = overlay.truecolor.value_or(true);
...
{
    if (!settings.showTimeSaved || summary.cacheHitsSkipped == 0)
    {
        return {};
    }

    const std::string Message = "Saved time on " + std::to_string(summary.cacheHitsSkipped) +
                                (summary.cacheHitsSkipped == 1 ? " cached step" : " cached steps");
